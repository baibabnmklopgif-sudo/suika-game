...
- 水果采用**无角速度模型**：避免堆叠接触中的切向冲量误差累积而产生“原地转圈”；仅使用幅度受限的缩放弹簧实现撞击时的 Q 弹感觉，面部不会明显变形。
- 采用**速度自适应子步**：常规帧 4 子步，高速下落时才使用 6 子步，兼顾低端移动设备帧率与高速碰撞稳定性。
- 碰撞根据相对法线速度使用不同弹性系数：快速撞击有可见但克制的回弹，低速堆叠自然贴合。
- `localStorage` 被安全封装；若 App WebView 禁用了 DOM Storage，游戏仍会正常启动，仅不会跨会话保存金币/最高分。
